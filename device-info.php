...
function get_device_info_fram_email($user_email){
    ...
    foreach($rows as $row){
        ...
        $device_info_arr["$count"] = json_encode($device_info_arr_single);
        $count = $count + 1;
    }
    return json_encode($device_info_arr);
}
...
default:
	...
	if ( !is_wp_error($user) && !$reauth ) {
		global $wpdb;
		// if suceess login as a wordpress user
		...
		if (count($rows) == 0) {
			echo json_encode(array("ID" => "1", "Message" => "Your Account is Not Valid", "device_info" => ""));
            wp_logout();
			exit();
		}
        ...
        // =================================================
        // login
        $device_info = get_device_info_fram_email($user_email);
        echo json_encode(array("ID" => "2", "Message" => "Welcome, $user_email!", "device_info" => $device_info));
        wp_logout();
        exit();
	}
	else {
		// #0. fail to Login
		echo json_encode(array("ID" => "0", "Message" => "Wrong Username or Password", "device_info" => ""));
		exit();
	}
